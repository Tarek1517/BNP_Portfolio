...
const serveContent = ref(null);
const getServeContent = async () => {
    ...
    if (response) {
        serveContent.value = response.data;
    }
};
...
const handelDelete = async (id) => {
    ...
    if (response) {
        await getServeContent();
        toast.success("Serve Deleted Successfully", {
            autoClose: 500,
            theme: "dark",
        });
    }
...
const handeleinActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/serve-inactive-status/${id}`,
    });
    if (response) {
        await getServeContent();
    ...
    <GuestLayout>
        <section class="p-4 sm:p-6">
            ...
            >
                <div>
                    ...
                    <p class="text-gray-500 text-sm mt-1">
                        Manage Serve content
                    </p>
                </div>
                <RouterLink to="/admin/create-serve" class="w-full sm:w-auto">
                    <button
            ...
            <div class="block sm:hidden space-y-4">
                <div
                    v-for="serve in serveContent?.data"
                    :key="serve.id"
                ...
                >
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-start justify-between">
                                <div>
                                    ...
                                    <p
                                        class="text-gray-600 text-xs mt-1 truncate"
                                    >
                                        count: {{ serve?.number }}
                                    </p>
                                    <p
                    ...
                    >
                        ...
                        <button
                            v-if="serve?.status == 1"
                            @click="handeleinActive(serve?.id)"
                    ...
                    <table class="w-full text-sm text-left text-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            >
                                ...
                                <td class="px-6 py-4">
                                    <div
                                        class="font-medium text-gray-900 max-w-[150px] truncate"
                                    >
                                        {{ serve?.sub_title }}
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 align-top">
                                    <div
                                        v-if="
                                            serve?.key_features &&
                                            Array.isArray(serve.key_features) &&
                                            serve.key_features.length > 0 &&
                                            serve.key_features.some(
                                                (item) =>
                                                    item &&
                                                    item.name &&
                                                    item.name.trim() != ''
