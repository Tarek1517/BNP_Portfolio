...
const handeleinActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/policy-inactive-status/${id}`,
    });
    if (response) {
        await getPolicyContent();
...
const handeleActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/policy-active-status/${id}`,
    });
    if (response) {
        await getPolicyContent();
        toast.success("Status Update Successfully", {
    ...
    <GuestLayout>
        <section class="p-4 sm:p-6">
            ...
            >
                ...
                <RouterLink to="/admin/create-policy" class="w-full sm:w-auto">
                    <button
            ...
            <div class="block sm:hidden space-y-4">
                <div
                    v-for="policy in policyContent?.data"
                    :key="policy.id"
                ...
                >
                    ...
                    >
                        ...
                        <button
                            v-if="policy?.status == 1"
                            @click="handeleinActive(policy?.id)"
                    ...
                    <table class="w-full text-sm text-left text-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            >
                                ...
                                <td class="px-6 py-4">
                                    <div
                                        class="font-medium text-gray-900 max-w-[250px] truncate"
                                    >
                                        {{ policy?.title }}
                                    </div>
                                </td>
