...
const counterContent = ref(null);
const getCounterContent = async () => {
    ...
    if (response) {
        counterContent.value = response.data;
    }
};
...
const handeleinActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/counter-inactive-status/${id}`,
    });
    if (response) {
        await getCounterContent();
...
const handeleActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/counter-active-status/${id}`,
    });
    if (response) {
        await getCounterContent();
        toast.success("Status Update Successfully", {
    ...
    <GuestLayout>
        <section class="p-4 sm:p-6">
            ...
            >
                ...
                <RouterLink to="/admin/create-counter" class="w-full sm:w-auto">
                    <button
            ...
            <div class="block sm:hidden space-y-4">
                <div
                    v-for="counter in counterContent?.data"
                    :key="counter.id"
                ...
                >
                            ...
                            <div class="flex items-start justify-between">
                                <div>
                                    ...
                                    <p
                                        class="text-gray-600 text-xs mt-1 truncate"
                                    >
                                        icon: {{ counter?.number_icon }}
                                    </p>
                                </div>
                                <div>
                                    <span
                                        v-if="counter?.status == 1"
                    ...
                    >
                        ...
                        <button
                            v-if="counter?.status == 1"
                            @click="handeleinActive(counter?.id)"
                        ...
                        <button
                            v-else
                            @click="handeleActive(counter?.id)"
                        ...
                        <RouterLink
                            :to="`/admin/edit-counter/${counter?.id}`"
