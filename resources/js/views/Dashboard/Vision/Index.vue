...
const handeleinActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/vision-inactive-status/${id}`,
    });
    if (response) {
        await getVisionContent();
...
const handeleActive = async (id) => {
    const response = await sendRequest({
        method: "get",
        url: `/v1/vision-active-status/${id}`,
    });
    if (response) {
        await getVisionContent();
        toast.success("Status Update Successfully", {
    ...
    <GuestLayout>
        <section class="p-4 sm:p-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
                ...
                <RouterLink to="/admin/create-vision" class="w-full sm:w-auto">
                    <button
            ...
            <div class="block sm:hidden space-y-4">
                <div v-for="vision in visionContent" :key="vision.id"
                                ...
                                <div>
                                    ...
                                    <p class="text-gray-600 text-xs mt-1 truncate">
                                         <img class="w-12 h-12 sm:w-16 sm:h-16 object-cover rounded-lg"
                                            :src="vision?.image" :alt="vision?.title">
                                    </p>
                                </div>
                                <div>
                                    <div v-if="
                                        vision?.key_features &&
                                        Array.isArray(vision.key_features) &&
                                        vision.key_features.length > 0 &&
                                        vision.key_features.some(
                                            (item) =>
                                                item &&
                                                item.quote &&
                                                item.quote.trim() != ''
                    ...
                    <div class="flex items-center gap-2 pt-4 border-t border-gray-100">
                        <!-- Status Toggle -->
                        <button v-if="vision?.status == 1" @click="handeleinActive(vision?.id)"
                        ...
                        <button v-else @click="handeleActive(vision?.id)"
                        ...
                        <RouterLink :to="`/admin/edit-vision/${vision?.id}`"
                        ...
                        <button @click="handelDelete(vision?.id)"
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-700">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="vision in visionContent" :key="vision.id"
